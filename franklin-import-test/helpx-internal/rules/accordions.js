...
export default function createAccordionBlocks(main, document) {
  ...
  accordions.forEach((accordion) => {
    const accContent = accordion.querySelector('.spectrum-Accordion');

    if (!accContent) {
      return;
    }

    const items = accContent.querySelectorAll(':scope > .spectrum-Accordion-item');
    ...
    if (items) {
      ...
      accContent.insertAdjacentElement('beforebegin', accBlockTable);
